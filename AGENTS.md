...
- Windows install target is `nsis` (`ScreenDesc-Setup.exe`, wizard-style, per-user, no admin required) — electron-updater doesn't support update-over-zip on Windows, so there's no separate Windows zip.
- macOS ships both `dmg` (drag-to-Applications installer for first-time downloads, not itself update-aware) and `zip` (the Squirrel.Mac update payload `latest-mac.yml` points at — kept in the Release for the updater, not meant as a user-facing download).
- `package.json`'s top-level `dependencies` must stay limited to packages `desktop-electron/*.cjs` actually `require()`s at runtime (currently just `electron-updater`) — `build.files` has no `node_modules` exclusion, so electron-builder's own production-dependency-tree resolution decides what ships in `app.asar`, and it walks everything listed there. Frontend libraries (`vue`, `pinia`, `onnxruntime-web`, etc.) belong in `devDependencies`: Vite bundles them into `dist/` for both the web and desktop builds, so the Electron main process never needs the raw package on disk. Getting this wrong doesn't fail the build — it either bloats every installer with megabytes of unused packages, or (worse) omits a package the main process does need, which only surfaces as a `Cannot find module` crash in the packaged app, not in `npm run desktop:run`.
